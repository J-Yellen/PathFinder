...
1. **`pathfinder/matrix_handler.py`** - `BinaryAcceptance` class converts correlation/relation matrices to graph representation
   ...
   - **Key method**: `sort_bam_by_weight()` returns index map for remapping results back to original order
...
3. **`pathfinder/result.py`** - `Results` class manages path collections
   ...
   - `remap_path(index_map)` reverses the sorting done by `sort_bam_by_weight()`
...
**Critical**: When using sorted weights (via `sort_bam_by_weight()`), results are in sorted index space. Must call `remap_path(index_map)` to get original indices.
